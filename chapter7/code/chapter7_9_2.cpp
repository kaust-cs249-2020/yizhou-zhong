...
const char outputFileName[20] = "chapter7_9_2.txt";
...
int main(int argc, char *argv[])
{
	...
	// operation
	vector<vector<vector<int>>> sequence = TwoBreakSorting(genoP, genoQ);
	...
	for (int i = 0; i < sequence.size();i++)
	{
		for (int j = 0; j < sequence[i].size();j++)
		{
			cout << '(';
			outfile << '(';
			cout << SequenceFormat(sequence[i][j]);
			outfile << SequenceFormat(sequence[i][j]);
			cout << ')';
			outfile << ')';
		}
		cout << endl;
